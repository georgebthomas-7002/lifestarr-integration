...
2. **Mighty fires `MemberJoined` per-space** — when a member joins a community OR is added to any Space within it, Mighty fires `MemberJoinedHook` with that `space_id` in the top-level payload. So a single new member can fire 20+ MemberJoined events in a burst. Same for `MemberLeftHook` when they leave a space. We treat each fire as idempotent and use it to populate `lifestarr_spaces`. **Caveat:** the very first fire (the community signup itself) carries `space_id == network_id` — i.e., the network/community id where no real space exists. Both handlers detect that case and skip the `lifestarr_spaces` write; otherwise HubSpot 400s with `INVALID_OPTION` because the network id isn't in the multi-select option list. Same defensive guard skips any unknown `space_id` not present in `lib/space-config.ts SPACE_NAMES`, with a `console.warn` so new spaces get noticed.
